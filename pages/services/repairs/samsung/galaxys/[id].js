    ...
    <Layout pageName={phone[0].model} Description={phone[0].description}>
      <main className="container mx-auto p-5">
        <section className="flex gap-10 flex-col justify-center items-center p-5">
          ...
          <Image
            src={'/images/services/repairs/samsung/galaxys/' + phone[0].src}
            alt={phone[0].model}
            width={200}
            height={200}
          />
...
export async function getStaticPaths() {
  ...
  let data = await db.collection('listgalaxys').find({}).toArray();
...
export async function getStaticProps({params}) {
  ...
  let data = await db.collection('galaxys').find({href: params.id}).toArray();
  data = JSON.parse(JSON.stringify(data));
