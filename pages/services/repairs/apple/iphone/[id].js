      ...
      <main className="flex flex-col my-10 md:my-24 justify-center items-center mx-5 md:mx-16">
        <div className="flex flex-col md:flex-row ">
          ...
          <section className="flex gap-3 md:gap-10 flex-col justify-center items-center p-5 ">
            <h1 className="text-2xl">{phone[0].model}</h1>
            <Image
              src={"/images/services/repairs/apple/iphone/" + phone[0].image}
              alt={phone[0].model}
              width={200}
              height={200}
            />
...
export async function getServerSideProps({ params }) {
  ...
  let data = await db.collection("iphone").find({ url: params.id }).toArray();
